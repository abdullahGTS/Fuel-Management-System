...
exports.ReloadAlarmsCharts = void 0;
...
var AlarmFilter = {
  ...
  init: function init() {
    var dtFilterWrapper, alarms, filterOptions, selectedFilters;
      ...
      while (1) {
        switch (_context.prev = _context.next) {
          ...
          case 8:
            ...
            AlarmFilter.state.alarms = alarms; // Get all unique filter categories from the response

            filterOptions = {
              'Site Numbers': {
                options: _toConsumableArray(new Set(alarms.map(function (item) {
            ...
            AlarmFilter.state.filterOptions = filterOptions; // Initialize filters with the fetched alarm data

            _context.next = 13;
            return regeneratorRuntime.awrap(_script.DatatableFilter.init(dtFilterWrapper, alarms, filterOptions, AlarmFilter, AlarmDT));
  ...
  filterSubmit: function filterSubmit(filters) {
    var alarms, filteredAlarms;
    return regeneratorRuntime.async(function filterSubmit$(_context2) {
      while (1) {
        switch (_context2.prev = _context2.next) {
          case 0:
            alarms = AlarmFilter.state.alarms;
            _context2.next = 3;
            return regeneratorRuntime.awrap(AlarmFilter.applyFilters(filters, alarms));

          case 3:
            filteredAlarms = _context2.sent;
  ...
  applyFilters: function applyFilters(filters, alarms) {
    var filteredAlarms = alarms; // Start with the full alarm list
    ...
    Object.entries(filters).forEach(function (_ref) {
      ...
      if (key === 'time' && values.from && values.to) {
        ...
        filteredAlarms = filteredAlarms.filter(function (alarm) {
          var alarmDate = new Date(alarm[key]);
        ...
        filteredAlarms = filteredAlarms.filter(function (alarm) {
          return values.some(function (value) {
            return String(alarm[key]) === String(value);
          });
        });
      }
    });
      ...
      while (1) {
        switch (_context4.prev = _context4.next) {
          case 0:
            _context4.prev = 0;
            _context4.next = 3;
            return regeneratorRuntime.awrap(_script.DataTable.fetchData(_constant.API_PATHS.alarmData));

          case 3:
            alarms = _context4.sent;
            return _context4.abrupt("return", alarms);

          case 7:
            _context4.prev = 7;
            _context4.t0 = _context4["catch"](0);
            console.error("Error fetching AlarmDT data:", _context4.t0);
...
exports.ReloadAlarmsCharts = ReloadAlarmsCharts;
var RunCharts = {
  init: function init() {
    AlarmSeverity.init();
