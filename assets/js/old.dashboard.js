//dashboard.js
import { pageReady } from './script.js';
import { API_PATHS, fetchData } from './constant.js';
...
const GetCurrentProductValue = {
    ...
    init: () => {
        const gasoline95 = document.querySelector('#gasoline95-value');
        const gasoline91 = document.querySelector('#gasoline91-value');
        const diesel = document.querySelector('#diesel-value');

        if ( gasoline95 ) GetCurrentProductValue.currentGasoline95Value = GetCurrentProductValue.getCurrentValue('gasoline95-value');
        if ( gasoline91 ) GetCurrentProductValue.currentGasoline91Value = GetCurrentProductValue.getCurrentValue('gasoline91-value');
        if ( diesel ) GetCurrentProductValue.currentDieselValue = GetCurrentProductValue.getCurrentValue('diesel-value');
    }
};
...
pageReady(() => {
    ...
    console.log( API_PATHS.alarmsOperationalPieCharts )
});